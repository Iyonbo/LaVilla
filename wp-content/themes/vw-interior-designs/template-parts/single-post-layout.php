    ...
    <div class="single-post">
        ...
        <?php } ?> 
            <div class="entry-content">
                <?php the_content(); ?>
                <?php if(get_theme_mod('vw_interior_designs_toggle_tags',true)==1){ ?>
        ...
        <?php
            ...
            if ( is_singular( 'attachment' ) ) {
                ...
                the_post_navigation( array(
                    'prev_text' => _x( '<span class="meta-nav">Published in</span><span class="post-title">%title</span>', 'Parent post link', 'vw-interior-designs' ),
                ) );
            } elseif ( is_singular( 'post' ) ) {
                // Previous/next post navigation.
                the_post_navigation( array(
                    'next_text' => '<span class="meta-nav" aria-hidden="true">' .esc_html(get_theme_mod('vw_interior_designs_single_blog_next_navigation_text','NEXT')) . '</span> ' .
                        '<span class="screen-reader-text">' . __( 'Next post:', 'vw-interior-designs' ) . '</span> ' .
                        '<span class="post-title">%title</span>',
                    'prev_text' => '<span class="meta-nav" aria-hidden="true">' .esc_html(get_theme_mod('vw_interior_designs_single_blog_prev_navigation_text','PREVIOUS')) . '</span> ' .
                        '<span class="screen-reader-text">' . __( 'Previous post:', 'vw-interior-designs' ) . '</span> ' .
                        '<span class="post-title">%title</span>',
                ) );
            }
        ?>
